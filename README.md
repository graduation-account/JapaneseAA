...
This is a Gradio application that analyzes Japanese text and calculates similarity with selected authors using pre-trained BERT+CatBoost ensemble models.
...
ブラウザで http://127.0.0.1:7860 にアクセスして使用できます。
...
Access http://127.0.0.1:7860 in your browser to use the application.
...
5. **結果の確認**
   - 右側の「Result」パネルに以下の情報が表示されます：
     - 選択した著者との類似度スコア（パーセンテージ）
     - すべての著者に対する予測確率
     - 分析結果（最大確率が50%未満の場合は「Unknown Author」の可能性が表示されます）

### English

1. **Select Author Group**
   - Choose the author group from "Author Group Selection"
...
2. **Enter Text**
   - Input the Japanese text you want to analyze in the "Input Text" field
...
5. **View Results**
   - The "Result" panel on the right displays:
     - Similarity score with the selected author (percentage)
     - Prediction probabilities for all authors
...
5. **類似度表示**
   ...
   - すべての著者に対する予測確率を降順で表示します
   - 最大確率が50%未満の場合は「Unknown Author」の可能性を警告します
...
5. **Similarity Display**
   ...
   - Shows prediction probabilities for all authors in descending order
